...
import { RoleType, useAuth } from "../context/AuthContext";
...
const searchItems: SearchItem[] = [
  { label: "Dashboard", path: "/", section: "General" },
  { label: "User Profile", path: "/profile", section: "General" },
        ...
        <div className="flex items-center justify-between w-full gap-2 px-3 py-3 border-b border-gray-200 dark:border-gray-800 sm:gap-4 lg:justify-normal lg:border-b-0 lg:px-0 lg:py-4">
          ...
          <div className="hidden lg:block">
            <form
              onSubmit={(event) => {
                event.preventDefault();
                if (searchResults[0]) openSearchResult(searchResults[0].path);
              }}
            >
              <div className="relative">
                ...
                {searchQuery.trim() && (
                  <div className="absolute left-0 right-0 top-full z-50 mt-2 overflow-hidden rounded-lg border border-gray-200 bg-white py-1 shadow-theme-lg dark:border-gray-800 dark:bg-gray-900">
                    {searchResults.length ? searchResults.map((item) => (
                      <button
                        key={item.path}
                        type="button"
                        onClick={() => openSearchResult(item.path)}
                        className="flex w-full items-center justify-between px-4 py-2.5 text-left hover:bg-gray-100 dark:hover:bg-white/5"
                      >
                      ...
                      <p className="px-4 py-3 text-sm text-gray-500 dark:text-gray-400">No matching pages found.</p>
                    )}
                  </div>
                )}
              </div>
